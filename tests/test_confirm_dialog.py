...
class TestConfirmDialogIntegration:
    ...
    def test_confirm_dialog_bindings_structure(self) -> None:
        ...
        assert len(dialog.BINDINGS) == 4
        assert all(binding.key in ["y", "n", "enter", "escape"] for binding in dialog.BINDINGS)
    ...
    def test_confirm_dialog_yes_shortcut(self) -> None:
        ...
        yes_bindings = [b for b in dialog.BINDINGS if b.key == "y"]
        assert len(yes_bindings) == 1
        assert "confirm" in yes_bindings[0].action
    ...
    def test_confirm_dialog_no_shortcut(self) -> None:
        ...
        no_bindings = [b for b in dialog.BINDINGS if b.key == "n"]
        assert len(no_bindings) == 1
        assert "cancel" in no_bindings[0].action
    ...
    def test_confirm_dialog_escape_closes(self) -> None:
        ...
        escape_bindings = [b for b in dialog.BINDINGS if b.key == "escape"]
        assert len(escape_bindings) == 1
        assert "cancel" in escape_bindings[0].action
